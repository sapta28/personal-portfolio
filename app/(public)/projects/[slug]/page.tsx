...
function renderMarkdownToJSX(content: string) {
  ...
  const flushList = (key: number) => {
    if (listItems.length > 0) {
      jsxElements.push(
        <ul key={`list-${key}`} className="list-disc pl-6 my-4 space-y-2 text-slate-650 text-xs sm:text-sm">
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmedLine.startsWith('- ')) {
      ...
      if (trimmedLine.startsWith('### ')) {
        jsxElements.push(
          <h3 key={index} className="text-xl font-bold text-slate-800 mt-8 mb-4 border-b border-slate-100 pb-2">
            {trimmedLine.substring(4)}
          </h3>
        )
      } else if (trimmedLine.startsWith('#### ')) {
        jsxElements.push(
          <h4 key={index} className="text-base font-bold text-primary mt-6 mb-3">
        ...
        jsxElements.push(
          <p
            key={index}
            className="text-slate-600 text-xs sm:text-sm leading-relaxed mb-4"
...
export default async function ProjectDetailPage({ params }: ProjectDetailPageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 space-y-8">
      ...
      <Link
        href="/projects"
        className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-primary transition-colors group mb-4"
      >
      ...
      {/* Banner / Gambar */}
      <div className="relative aspect-video w-full overflow-hidden rounded-quad border border-slate-200 bg-slate-100">
        <img
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 pt-4">
        ...
        <div className="space-y-6">
          <div className="glass p-6 rounded-quad space-y-6 shadow-sm">
            ...
            <div className="flex flex-col gap-3">
              {project.github_url ? (
                <a
                  ...
                  className="flex items-center justify-between w-full px-4 py-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-xs text-slate-700 font-semibold rounded-quad transition-colors"
                >
              ...
              {project.live_url ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-between w-full px-4 py-3 bg-primary hover:bg-primary-hover text-xs text-white font-bold rounded-quad transition-colors shadow-sm"
                >
                  <span className="flex items-center gap-2">
          ...
          <div className="glass p-6 rounded-quad shadow-sm">
            <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider border-b border-slate-100 pb-2 mb-4">
              Informasi Teknis
            </h3>
            <div className="space-y-2 text-xs text-slate-600">
              <p><strong className="text-slate-700">Skalabilitas:</strong> Teruji di skala produksi lokal.</p>
              <p><strong className="text-slate-700">Integrasi:</strong> MQTT & REST API.</p>
              <p><strong className="text-slate-700">Database:</strong> PostgreSQL / Supabase.</p>
            </div>
          </div>
        </div>
      </div>
...
export function generateMetadata({ project }: { project: Project }) {
  if (!project) return {}
  return {
    title: `${project.title} | Sapta Portfolio`,
